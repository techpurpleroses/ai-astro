...
const logger = createServerLogger("api.dashboard.features.magic-ball");

export async function GET(req: NextRequest) {
  return observeApiRoute({
    scope: "api.dashboard.features.magic-ball.GET",
    request: req,
    handler: async () => {
      try {
        const { tarotService } = getAstroAiRuntime();
        const result = await tarotService.getMagicBallPool();
