...
const logger = createServerLogger("api.dashboard.features.stories");

export async function GET(req: NextRequest) {
  return observeApiRoute({
    scope: "api.dashboard.features.stories.GET",
    request: req,
    handler: async () => {
      try {
        const { storiesService } = getAstroAiRuntime();
        const result = await storiesService.listCategories();
