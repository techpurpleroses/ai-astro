...
const logger = createServerLogger("api.dashboard.features.reports.slug");
...
export async function GET(
  ...
  return observeApiRoute({
    scope: "api.dashboard.features.reports.slug.GET",
    request: req,
    handler: async () => {
      try {
        ...
        const result = await reportsService.getProductWithDetail(slug);
