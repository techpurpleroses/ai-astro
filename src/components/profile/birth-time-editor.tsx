...
export function BirthTimeEditor() {
  ...
  const { data: profile } = useQuery<ProfileData>({
    queryKey: ['profile'],
    queryFn: () =>
      astroFetchJson('/api/dashboard/profile', { debugOrigin: 'components.profile.birth-time' }),
    staleTime: 60_000,
  })
  ...
  useEffect(() => {
    if (profile?.birthTime && !time) {
      setTime(profile.birthTime)
    }
  }, [profile, time])

  async function handleSave() {
    if (saving || !time) return
    setSaving(true)
    setError(null)
    try {
      await astroFetchJson('/api/dashboard/profile', {
        debugOrigin: 'components.profile.birth-time.save',
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ birthTime: time }),
      })
  ...
  return (
    <div className="flex flex-col min-h-full">
      ...
      >
        ...
        <h1 className="font-mystical text-2xl text-[#F4E2B4] flex-1 text-center mr-8">Time of Birth</h1>
      </div>

      <div className="px-5 pt-8 flex flex-col gap-4">
        <p className="text-sm text-text-muted">
          Your birth time helps us calculate your Ascendant and accurate house placements
        </p>
