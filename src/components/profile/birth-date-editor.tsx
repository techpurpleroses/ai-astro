...
export function BirthDateEditor() {
  ...
  const { data: profile } = useQuery<ProfileData>({
    queryKey: ['profile'],
    queryFn: () =>
      astroFetchJson('/api/dashboard/profile', { debugOrigin: 'components.profile.birth-date' }),
    staleTime: 60_000,
  })
  ...
  async function handleSave() {
    ...
    try {
      await astroFetchJson('/api/dashboard/profile', {
        debugOrigin: 'components.profile.birth-date.save',
        method: 'PATCH',
  ...
  return (
    <div className="flex flex-col min-h-full">
      ...
      >
        ...
        <h1 className="font-mystical text-2xl text-[#F4E2B4] flex-1 text-center mr-8">Date of Birth</h1>
      </div>

      <div className="px-5 pt-8 flex flex-col gap-4">
        <p className="text-sm text-text-muted">Your birth date is used to calculate your sun sign and horoscope</p>
