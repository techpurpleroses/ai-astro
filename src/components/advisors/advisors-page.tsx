...
import { Star, MessageCircle, Clock, Zap, Heart, ChevronRight } from 'lucide-react'
  ...
  return (
    <div className="glass-card rounded-2xl p-4 flex gap-3 items-start active:bg-white/5 transition-colors">
      <div className="relative shrink-0">
        <div className="h-14 w-14 rounded-2xl flex items-center justify-center text-lg font-display font-bold"
          style={{ background: 'linear-gradient(135deg, rgba(6,182,212,0.25), rgba(167,139,250,0.15))' }}
        >
          {initials}
        </div>
        <div className="absolute -bottom-0.5 -right-0.5 h-3.5 w-3.5 rounded-full border-2 border-midnight-800"
      ...
      <div className="flex-1 min-w-0">
        ...
        >
          {advisor.isOnline ? (
            <><Zap size={11} className="text-cyan-glow" />Chat Now</>
          ) : (
            <><Clock size={11} />Leave Message</>
          )}
        </button>
      </div>
    </div>
...
function MyChatsStrip({ advisors, recentIds, onSelect }: { advisors: Advisor[]; recentIds: string[]; onSelect: (id: string) => void }) {
  const recent = recentIds.map((id) => advisors.find((a) => a.id === id)).filter(Boolean) as Advisor[]
  if (!recent.length) return null
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between px-4">
        <p className="text-[11px] font-display font-semibold text-text-primary">My chats</p>
        <button className="text-[10px] text-cyan-glow font-display">All advisors ›</button>
      </div>
          ...
          return (
            <button key={advisor.id} onClick={() => onSelect(advisor.id)} className="flex flex-col items-center gap-1.5 shrink-0">
              <div className="relative">
                <div className="h-14 w-14 rounded-full flex items-center justify-center text-sm font-display font-bold"
...
export function AdvisorsClient() {
  ...
  return (
    <div className="flex flex-col">
      {/* Header: user avatar left · Advisors center · heart+count right */}
      <div className="sticky top-0 z-30 px-4 py-3 flex items-center"
      ...
      >
        ...
        <button className="flex items-center gap-1 px-2 py-1 rounded-full shrink-0"
      ...
      {data && (
        <div className="space-y-5 pt-4 pb-6">
          {/* My chats horizontal strip */}
          <FadeIn delay={0}>
            <MyChatsStrip advisors={data.advisors} recentIds={data.recentChats} onSelect={handleChat} />
          </FadeIn>

          {/* Ask question input */}
          <FadeIn delay={0.04}>
            <AskQuestionCTA onAsk={() => {}} />
          </FadeIn>
            ...
            <div className="px-4 space-y-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[9px] font-display font-bold text-lime-accent bg-lime-accent/10 border border-lime-accent/25 px-2 py-0.5 rounded-full uppercase tracking-wider">
                  {data.advisors.filter((a: Advisor) => a.isOnline).length} Live
                </span>
              </div>
              {data.advisors
                .filter((a: Advisor) => a.isOnline)
                .map((advisor: Advisor) => (
          ...
          {/* Offline advisors */}
          {data.advisors.some((a: Advisor) => !a.isOnline) && (
            <FadeIn delay={0.14}>
              <div className="px-4 space-y-3">
